...
session_start();
$state 		= explode('_',$_GET['state']);
 $uid 			= $state[0];
 $conditions 	= array("id"=>$uid);
 $_SESSION['domain'] = $state[1];
...
if (null != $user) { 
 ...
 $appid 		= 'wx68aef7d7e4bfbc2b';
 $appsecret 	= '07fb27658e9b80f7d66e9e84b585234b';
 $code			= $_GET['code'];
$cz_openid		= $user[0]['cz_openid'];
...
if (getenv("HTTP_CLIENT_IP"))  
 ...
 if(!$cz_openid){
	 $cz_openid = getOpenid($appid,$appsecret,$code);
	 //file_put_contents('a.txt',$code);
	 db_update('js_user', array('cz_openid'=>$cz_openid),array('id'=>$uid),$state[1]);
 }
		...
		<script type="text/javascript">
			function submit_api(){
				...
				if(amount<100){
					...
					$.ajax({
						url:"/pay/jspay/request.php?method=submitOrderInfo",
						type:'post',
						dataType:'json',
								...
								WeixinJSBridge.invoke('getBrandWCPayRequest',{
									...
									"paySign" : data.pay_info.paySign //微信签名
										...
										if(res.err_msg == "get_brand_wcpay_request:ok" ) {
											open_info_msg("充值成功，充值金额为："+total_fee+"元");
										}
									});
