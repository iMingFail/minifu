        ...
        <div class="admin-content-body">
            <div class="am-cf am-padding am-padding-bottom-0">
                <div class="am-fl am-cf">
                    <strong class="am-text-primary am-text-lg">团队交易管理</strong> / <small>列表(<?php if($isModel == 1) {echo "实盘";}else{echo "模拟盘";} ?>)</small>
                </div>
            ...
            <div class="am-g">
                <div class="am-u-sm-12">
                    <table class="am-table am-table-striped am-table-hover table-main" id="doc-modal-list">
                        ...
                        <tbody>
                        <?php
                        $user_id = $_SESSION["agent_admin"]['id'];
                        //获取自己的所有下线
                        $conditions111 = array('pid' => $_SESSION["agent_admin"]['id']);
                        $params11 = "id";
                        $result11 = db_select("js_user", $conditions111, $params11);
                        $where = '';
                        foreach($result11 as $k=>$v){
                            $where .= ' user_id = '.$v[id].' or ';
                        }
                        $where = substr($where,0,-3);
                        $conditions = 'isModel = '.$isModel.' and ('. $where.')';                       
                        
                        
                        $conditions = array("isModel"=>$isModel , '('. $where.')'=>'');
                        $params = "b.realName,insert(b.mobilePhone,4,4,'****') mobilePhone,c.name";
						...
						$total_result = db_select("js_order", $conditions, $params, null, array("a.begin_time"=>"desc"), $left_join_tab);
						if ($total_result != null && count($total_result) > 0) {
                            for ($i = 0; $i < count($total_result); $i++) {
								$total_order_price += $total_result[$i]["order_price"];
							}
						}
						
                        if ($result != null && count($result) > 0) {
                            for ($i = 0; $i < count($result); $i++) {
								$page_order_price += $result[$i]["order_price"];
                                ?>
                    ...
                    <div class="am-cf">
                        ...
                        if ($pageNo >= 1) {
							...
							总金额：<strong style="color:red;"><?php echo $total_order_price ?> 元</strong> &nbsp;&nbsp;&nbsp;&nbsp;
							当页总金额：<strong style="color:red;"><?php echo $page_order_price ?> 元</strong>
                        <?php } ?>
                        <div class="am-btn-toolbar am-fr">
                            <div class="am-btn-group am-btn-group-sm">
                                ...
                                //生成页码
                                $pageViewString = $p->echoPageAsDiv("teamorders.php?isModel=$isModel", "&");
                                echo $pageViewString;
                                ?>
                            </div>
                        </div>
