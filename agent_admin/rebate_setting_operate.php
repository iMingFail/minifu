<?php
    ...
    if ($id != 0) {
        ...
        $return_rates = $result[0]["return_rates"];
    }
    $action = "add";
        ...
        <div class="admin-content-body">
            <div class="am-cf am-padding am-padding-bottom-0">
                <div class="am-fl am-cf">
                    <strong class="am-text-primary am-text-lg"><a href="rebate_setting.php">返利设置</a></strong>
                </div>
            </div>
            <hr>
                        ...
                        <fieldset>
                            <form class="am-form user-info-form">
                                <div class="am-form-group">
                                    <label for="doc-vld-age-2">返利主题：</label>
                                    <input type="text" id="name" value="<?php echo $name?>" placeholder="输入返利主题" required/>
                                    <input type="hidden" id="id_txt" value="<?php echo $id?>" />
                                </div>
                                <div class="am-form-group">
                                    <label for="doc-vld-age-2">返利条件：</label>
                                    <input type="text" id="conditions" value="<?php echo $condition?>" placeholder="输入返利条件" required/>
                                </div>
                                <div class="am-form-group">
                                    <label for="doc-vld-url-2">返利率：</label>
                                    <input type="text" id="return_rates" value="<?php echo $return_rates?>" placeholder="输入返利率" required/>
                                </div>
                            </form>
        ...
        $(".user-info-form").validator({
            submit:function(){
                if (this.isFormValid()) {
                    ...
                    switch(action) {
                        case "add":
                            data = {"name":$("#name").val().trim(), "conditions":$("#conditions").val().trim(), "return_rates":$("#return_rates").val().trim()};
                            break;
                        case "edit_rebate_setting":
                            var name = $("#name").val().trim();
                            var conditions = $("#conditions").val().trim();
                            var return_rates = $("#return_rates").val().trim();
                            var id = $("#id_txt").val();
                            data = {"name":name, "conditions":conditions, "return_rates":return_rates, "id":id};
                            break;
                    }
                    $.ajax({
                        type:"POST",
                        url:"api/api.php?t=s&action="+action,
                        dataType:"json",
                        data:data,
                        beforeSend:function(){
                            loading("正在保存数据...");
