...
<script>
	...
	function load_water(num,PAGESIZE,listtype,isnull) {
		...
		$.ajax({
			type:"POST",
			url:"action/userAction.php?action=user_team_rebate_list",
			data : "page=" + num + "&pagesize=" + PAGESIZE + "&uid=" + uid,
			dataType:"json",
			success:function(json){
				...
				if (json.success == 1) {
					$("#all_list").removeClass("hide");
					if(isnull==1){
						$("#all_list").html('<tr class="rec-tr"><th class="rec-th">用户</th><th class="rec-th">下单时间</th><th class="rec-th">下单价格</th><th class="rec-th">盈利状态</th><th class="rec-th">返利</th><th class="rec-th">返利时间</th></tr>');
						$(".com-empty").addClass("hide");
					}
					$.each(json.data, function(key, value) {
						var order_price = 0;
						if (null != value.order_price && value.order_price != "") {
							order_price = value.order_price;
						}
						var price = 0;
						if (null != value.price && value.price != "") {
							price = value.price;
						}
						var vouchers_price = 0;
						if (null != value.vouchers_price && value.vouchers_price != "") {
							vouchers_price = value.vouchers_price;
						}
						var money = 0;
						...
						var state = "未开奖";
						if (null != value.state && value.state != "") {
							if (value.state == 1) {
								state = "盈利";
						...
						var datetime = "无";
						if (value.datetime != "" && value.datetime != null) {
							datetime = value.datetime;
						}
						var html = '<tr class="rec-tr"><td class="rec-td fcgray3"><a href='+value.uid+'"my_team_rebate.php?uid=">'+realName +"-"+value.mobilePhone+'</a></td>';
						html += '<td class="rec-td">'+value.begin_time+'</td>';
						html += '<td class="rec-td">'+price+'+'+vouchers_price+'</td>';
						html += '<td class="rec-td">'+state+'</td>';
						html += '<td class="rec-td">'+parseFloat(money).toFixed(2)+'</td>';
						html += '<td class="rec-td">'+datetime+'</td></tr>';
