...
function edit() {
    ...
    $sex = $_POST['sex'];
	$agent_power = $_POST['agent_power'];
    $nickName = $_POST['nickName'];
    $password = $_POST['password'];
    $arr = array();
    $params = array("identity"=>$identity, "realName"=>$realName, "sex"=>$sex, "nickName" => $nickName, "agent_power"=>$agent_power);
...
function top_up() {
    ...
    if ($r) {
        ...
        $sql = "insert into js_balance_log (userId, order_code, type, money, give_money, state, datetime,cz) value(".$id.",'".$order_code."',1, ".floatval($balance).",0,1,now(),1)";
        db_execute($sql);
        $arr['msg'] = '充值成功';
        echo json_encode($arr);
    } else {
