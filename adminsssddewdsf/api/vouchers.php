...
function generate() {
    ...
    for ($i=0;$i<$num;$i++) {
        ...
        $result = db_select('js_vouchers', $conditions);
        if ($result == null) {
            $params = array("number"=>$number, "password"=>"$password", "money"=>"$money", "user_id"=>$user_id, "state"=>1, "datetime"=>date('Y-m-d H:i'));
...
function delete() {
    if (!empty($_POST['ids']) && $_POST['ids'] != "")  {
        $ids = $_POST['ids'];
        $sql = "delete from js_vouchers where id in ($ids)";
